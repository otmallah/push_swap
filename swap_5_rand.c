...
void	rand5_num(t_stack *sta, t_stack *stb, t_stack *temp, t_iter *index)
{
	int	i;

	i = 0;
	ft_rand_100(sta, temp, index);
	while (i < 2)
	{
		if (i == sta->array[index->count3])
		{
			push_b(sta, stb, index);
			i++;
		}
		else if (i == sta->array[index->count3 + 1])
			swap_a(sta, index);
		else if (i == sta->array[index->count3 + 2])
		{
			rever_a(sta, index);
			rever_a(sta, index);
		}
		else if (i == sta->array[index->count3 + 3])
			r_rev_a(sta, index);
		else if (i == sta->array[index->count3 + 4])
			r_rev_a(sta, index);
	}
}
...
{
	rand5_num(sta, stb, temp, index);
	rand_num(sta, index);
	push_a(sta, stb, index);
	push_a(sta, stb, index);
}
